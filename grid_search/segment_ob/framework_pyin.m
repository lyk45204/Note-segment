...
%         tranProb_value = tranProb_value_trained;
        tranProb_value_pyin = [0.9, 0.99, 0.999, 0.9999];
        tranProb_value = tranProb_value_pyin;
...
parfor i_search = 1:n_search
    ...
    transProb_search = transProb{i_search};
...
%         waitbar(i_files/n_files,h,str)
        bestpath{i_files}  = hmmViterbi(obsProb{i_files}',transProb_search,init);
    end
...
Metrics_pyin = Metrics;
save('Metrics_pyin.mat','Metrics_pyin');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% rowname_ind = [8,12,16];
% [best_mat] = findbest(Metrics, rowname_ind, tranProb_value);
